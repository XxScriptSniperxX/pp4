...
HIGH QUALITY - cv2 Legend Detection + Crop + Resize Pipeline
"""
...
def cv2_detect_legend_bbox(cv_img: np.ndarray, fallback_ratio: float = LEGEND_FALLBACK_RATIO) -> tuple:
    """
    Detect legend bounding box using OpenCV.
    
    Algorithm:
    1. Convert to grayscale
    2. Apply threshold (inverse binary)
    3. Find contours
...
def crop_and_separate(cv_img: np.ndarray, legend_bbox: tuple) -> tuple:
    """
    Crop image into plot and legend based on detected bounding box.
    ...
    # Plot: everything left of legend
    plot_bgr = cv_img[:, :x]
    
    # Legend: exact bounding box
    legend_bgr = cv_img[y:y+ch, x:x+cw]
...
def resend(image: Image.Image, target_width_px: int = DEFAULT_LEGEND_SIZE[0], 
           ...
           bg_color: tuple = (255, 255, 255)) -> Image.Image:
    """
    Resize legend to fit target box with aspect ratio maintained.
...
def create_maneuver_slides(prs: Presentation, maneuver_id: str, figures: list,
    ...
    for idx, figdata in enumerate(figures):
        ...
        try:
            ...
            # STEP 5: CROP AND SEPARATE
            plot_bgr, legend_bgr = crop_and_separate(cv_img, legend_bbox)
            ...
            plot_resized = respect(plot_pil, DEFAULT_PLOT_SIZE)
            legend_resized = resend(legend_pil, *DEFAULT_LEGEND_SIZE)
